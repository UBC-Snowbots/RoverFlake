...
addChannelNetworkConnection(PhidgetChannelHandle channel, PhidgetNetConnHandle nc, uint16_t reqseq) {
	...
	MTAILQ_FOREACH(cnc, &channel->netconns, link) {
		if (cnc->nc == nc) {
			// This channel is already connected to this network connection.
			// The client probably timed out on open or something, so we need to update the setstatusrep or the client will never get the SetStatus packet
...
PhidgetReturnCode
openServerChannel(mosiop_t iop, uint64_t pid, Phidget_ChannelClass chclass, int chidx, int chreset, PhidgetNetConnHandle nc,
  PhidgetChannelHandle *openChannel, uint16_t reqseq) {
	...
	if (channel == NULL) {
		...
		// channel reset behaviour is set by the first client
		channel->openInfo->openCloseReset = chreset;
	} else {
...
PhidgetReturnCode
closeServerChannel(uint64_t pid, int chidx, int chreset, PhidgetNetConnHandle nc) {
	PhidgetChannelHandle channel;
	PhidgetDeviceHandle device;
	PhidgetReturnCode res;
	int cnt;
...
channelDeliverBridgePacket(PhidgetChannelHandle channel, BridgePacket *bp, PhidgetNetConnHandle nc,
  int forward) {
	...
	if (forward) {
		bridgePacketSetIsEvent(bp); /* We don't want to wait for each other client to respond - we are just notifying them that something happened. */
