...
PhidgetFirmwareUpgrade_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	const PhidgetUniqueDeviceDef *pdd;
	PhidgetFirmwareUpgradeHandle ch;
	Phidget_DeviceHardwareID vintId;
	int found;
	...
	switch (bp->vpkt) {
	case BP_DEVICEINFO:
		vintId = (Phidget_DeviceHardwareID)getBridgePacketInt32(bp, 0);
		...
		while (((int)pdd->type) != END_OF_LIST) {
			if (pdd->type == PHIDTYPE_VINT
			  && (unsigned short)vintId == pdd->vintID
			  && ((ch->actualDeviceVersion >= pdd->versionLow && ch->actualDeviceVersion < pdd->versionHigh) || ch->actualDeviceVersion == 0)) {
				ch->actualDeviceSKU = pdd->SKU;
				ch->actualDeviceName = pdd->name;
				ch->actualDeviceVINTID = pdd->vintID;
		...
		// If we didn't find it, it may be a version we don't know about. Just find any version with the same vintID
		//  - it's guaranteed to be compatible unless it's id 0x40, which mistakenly refers to 2 different devices
		if (!found && vintId != VINTID_0x40) {
			pdd = Phidget_Unique_Device_Def;
			while (((int)pdd->type) != END_OF_LIST) {
				if (pdd->type == PHIDTYPE_VINT && (unsigned short)vintId == pdd->vintID) {
					ch->actualDeviceSKU = pdd->SKU;
					ch->actualDeviceName = pdd->name;
					ch->actualDeviceVINTID = pdd->vintID;
					ch->actualDeviceID = pdd->id;
