...
PhidgetHub_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_SETFIRMWAREUPGRADEFLAG:
		TESTRANGE_IOP(bp->iop, "%d", getBridgePacketInt32(bp, 0), 0, (phid->parent->dev_hub.numVintPorts - 1));
...
PhidgetHub_getPortMode(PhidgetHubHandle ch, int port, PhidgetHub_PortMode *mode) {
	...
	TESTRANGE_PR(port, "%d", 0, (((PhidgetChannelHandle)ch)->parent->dev_hub.numVintPorts - 1));
...
API_PRETURN
PhidgetHub_getPortMaxSpeed(PhidgetHubHandle ch, int port, uint32_t *maxSpeed) {
	PhidgetHubDeviceHandle parentHub;
	PhidgetReturnCode res;
	...
	*maxSpeed = parentHub->portMaxSpeed[port];

	PhidgetRelease(&parentHub);

	if (*maxSpeed == PUNK_UINT32)
		return (PHID_RETURN(EPHIDGET_UNKNOWNVAL));
	return (EPHIDGET_OK);
}

API_PRETURN
PhidgetHub_getPortSupportsSetSpeed(PhidgetHubHandle ch, int port, int *state) {
	PhidgetHubDeviceHandle parentHub;
	PhidgetReturnCode res;
	...
	*state = parentHub->portSupportsSetSpeed[port];
...
API_PRETURN
PhidgetHub_getPortSupportsAutoSetSpeed(PhidgetHubHandle ch, int port, int *state) {
	PhidgetHubDeviceHandle parentHub;
	PhidgetReturnCode res;
	...
	*state = parentHub->portSupportsAutoSetSpeed[port];
